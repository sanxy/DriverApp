...
public class AddVehicleActivity extends AppCompatActivity implements VehicleSingleCheckListAdapter.OnItemClickListener {
    ...
    JSONArray storetypelist = new JSONArray();
    int vehicleTypeListSize;
    int storeTypeListSize;
    ...
    ArrayList<Boolean> deliverAllStoreTypesStatusArr;
...
//    String selectedtype = "";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        storeSelectArea = (LinearLayout) findViewById(R.id.storeSelectArea);
        ...
        exeWebServer.setDataResponseListener(responseString -> {
            ...
            if (responseStringObject != null && !responseStringObject.equals("")) {
                ...
                if (isDataAvail) {
                    ...
                    storetypelist = generalFunc.getJsonArray("storetypelist", message_obj);
                    if(storetypelist != null ){
                        storeTypeListSize = storetypelist.length();
                    }
...
//                    storeTypeListSize = storetypelist.length();
    ...
    public void buildServices(String IS_SHOW_VEHICLE_TYPE) {
        ...
        if (storeSelectArea.getChildCount() > 0) {
            storeSelectArea.removeAllViewsInLayout();
        }

        carTypesStatusArr = new ArrayList<>();
        rentalcarTypesStatusArr = new ArrayList<>();
        deliverAllStoreTypesStatusArr = new ArrayList<>();
        vehicletypelist_DeliverAll = new JSONArray();
        int position = 0;

        String[] vCarTypes = {};
        String[] vRentalCarType = {};
        String[] vServiceTypes = {};
        ...
        String vServiceType = getIntent().getStringExtra("vServiceType");
        if (vServiceType != null && !vServiceType.equals("")) {
            vServiceTypes = vServiceType.split(",");
        }
        ...
        if (vehicletypelist_DeliverAll.length() > 0 && generalFunc.isAnyDeliverOptionEnabled()) {
            ...
            for (int i = 0; i < storeTypeListSize; i++) {
                JSONObject obj = generalFunc.getJsonObject(storetypelist, i);
                String eType = generalFunc.getJsonValueStr("eType", obj);
                ...
                if (eType.equalsIgnoreCase(Utils.eSystem_Type)) {
                    ...
                    LayoutInflater inflater = (LayoutInflater) getActContext().getSystemService(Context.LAYOUT_INFLATER_SERVICE);
                    View view = inflater.inflate(R.layout.item_select_deliver_all_store, null);

                    MTextView serviceNameTxtView = (MTextView) view.findViewById(R.id.serviceNameTxtView);

                    serviceNameTxtView.setText(generalFunc.getJsonValueStr("vDeliverAllStoreType", obj));

                    final AppCompatCheckBox chkBox = (AppCompatCheckBox) view.findViewById(R.id.chkBox);

                    String iServiceId = generalFunc.getJsonValueStr("iServiceId", obj);

                    if (vServiceTypes != null && vServiceTypes.length > 0) {

                        String ischeck = generalFunc.getJsonValueStr("DeliverAllStoreStatus", obj);
                        if (ischeck.equalsIgnoreCase("true") || Arrays.asList(vServiceTypes).contains(iServiceId)) {
                            chkBox.setChecked(true);
                            deliverAllStoreTypesStatusArr.add(true);

                        } else {
                            deliverAllStoreTypesStatusArr.add(false);
                        }
                    } else {
                        deliverAllStoreTypesStatusArr.add(false);
                    }
                    ...
                    final int finalI = i;
                    chkBox.setOnCheckedChangeListener((buttonView, isChecked) -> {
                        deliverAllStoreTypesStatusArr.set(finalI, isChecked);


                    });
                    storeSelectArea.addView(view);
                }
    ...
    public void checkData() {
        ...
        boolean isStoreTypeSelected = false;
...
//        storeTypes = selectedCarTypes_DeliverAll;

        if (Utils.checkText(carTypes)) {
            isCarTypeSelected = true;
        }

        if (Utils.checkText(storeTypes)) {
        ...
        if (app_type.equals(Utils.CabGeneralType_UberX)) {
            ...
            for (int i = 0; i < deliverAllStoreTypesStatusArr.size(); i++) {
                if (deliverAllStoreTypesStatusArr.get(i)) {
                    isStoreTypeSelected = true;
                    ...
                    storeTypes = storeTypes.equals("") ? iServiceId : (storeTypes + "," + iServiceId);
                }
            }
            ...
            if (Utils.checkText(selectedCarTypes_DeliverAll)) {
                if (!isStoreTypeSelected) {
                    generalFunc.showMessage(generalFunc.getCurrentView((Activity) getActContext()), generalFunc.retrieveLangLBl(".", "LBL_SELECT_STORE_TYPE"));
                    return;
                }
            }
        ...
        if (iDriverVehicleId.equals("")) {
            if (ENABLE_EDIT_DRIVER_VEHICLE != null && ENABLE_EDIT_DRIVER_VEHICLE.equalsIgnoreCase("No")) {
                ...
                try {
                    ...
                    String finalStoreTypes = storeTypes;
                    editVehicleConfirmDialog.setBtnClickList(btn_id -> {
                        if (btn_id == 0) {
                            editVehicleConfirmDialog.closeAlertBox();
                        } else {
                            addVehicle(iSelectedMakeId, iSelectedModelId, finalCarTypes, finalStoreTypes);
                        }
                    });
                    editVehicleConfirmDialog.showAlertBox();
                } catch (Exception e) {
                    addVehicle(iSelectedMakeId, iSelectedModelId, carTypes, storeTypes);
                }
            } else {
                addVehicle(iSelectedMakeId, iSelectedModelId, carTypes, storeTypes);
            }
    ...
    public void addVehicle(String iMakeId, String iModelId, String vCarType, String
            vServiceType) {
        HashMap<String, String> parameters = new HashMap<String, String>();
        ...
        parameters.put("vServiceType", vServiceType);
                    ...
                    generateAlert.setBtnClickList(btn_id -> {
                        if (iDriverVehicleId.equals("")) {
                            if (btn_id == 0) {
                                ...
                                bn.putString("vServiceType", vServiceType);
