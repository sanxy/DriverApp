...
public class ConfigDriverTripStatus implements GetLocationUpdates.LocationUpdatesListener, UpdateFrequentTask.OnTaskRunCalled {
    ...
    public void startDriverStatusUpdateTask() {
        ...
        if (mContext != null) {
            ...
            Logger.d("ConfigDriverTripStatus", "ConfigDriver Time: " + FETCH_TRIP_STATUS_TIME_INTERVAL);
...
//                        releaseInstances();
    ...
    /* check if get details is update every time and do a refresh to show slide to arrive */
    private void updateDriverAcceptedStatus() {
        if (!intCheck.isNetworkConnected() && !intCheck.check_int()) {
            //callToJobFinish();
            return;
        }
        ...
        exeWebServer.setDataResponseListener(responseString -> {
            ...
            if (responseObj != null && !responseObj.equals("")) {
                ...
                Logger.d("ConfigDriverTrip", "Unable to process get details");
            }
        });
        exeWebServer.execute();
    ...
    private void dispatchMsg(String jsonMsg) {
        Logger.d("ConfigDriverTrip", "raw data: "+ jsonMsg);

        /* triggers the push notification to accept or decline trip */
